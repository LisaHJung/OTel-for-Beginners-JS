...
To observe your app or infrastructure, we first need to `instrument` it. 

In other words, we’re enabling it to generate traces, metrics, and logs.
...
`Automatic instrumentation` is ideal for getting started or when we can’t modify the app itself. 
- It automatically captures telemetry from your libraries and runtime environment, giving you useful visibility with very little setup.

`Manual instrumentation` gives you deeper insight by generating telemetry directly from your app. 
- Using the OpenTelemetry API, you can create custom traces, metrics, and logs that build on the data collected automatically.

In this episode, we’ll focus on auto-instrumentation to quickly generate traces and visualize them in Jaeger.
...
*This file prepares your app to generate traces through auto-instrumentation and send this trace data to any OTLP-compatible collector or backend.*
...
3. Configure auto-instrumentation and export traces to the local OpenTelemetry Collector.
...
These attributes will be addressed and cleaned up in the next episode using OpenTelemetry Collector processors:
- http.user_agent
- net.host.ip
- net.host.port
- net.peer.ip
...
Like the Tags section, this section also includes sensitive or low-value host and process details.
...
1. [`traces-bare-bones-setup`](https://github.com/LisaHJung/OTel-for-Beginners-JS/tree/traces-bare-bones-setup) (**Episode 4**)
...
2. [`traces-add-processors`](https://github.com/LisaHJung/OTel-for-Beginners-JS/tree/traces-add-processors) (**Current episode**)
- Uses the same setup as `traces-bare-bones-setup`, but applies processors to incoming traces.
...
### Step 1: Switch branch + restart app (Terminal 1)
In the terminal running the Roll the Dice App, stop the app, switch to the [`traces-add-processors`](https://github.com/LisaHJung/OTel-for-Beginners-JS/tree/traces-add-processors) branch, and restart the app.

```
# stop the app
CTRL + C

# switch to the traces-add-processors branch
